...
const FALLBACK = {
  ...
  '2026-12-25':'성탄절',
  // 2027 (네이버 캘린더 기준 — date.nager.at 폴백)
  '2027-01-01':'신정',
...
async function fetchNager(year){
  ...
  try {
    ...
    // 대체공휴일 — date.nager.at 누락분을 FALLBACK 정적 매핑에서 보강 (한국 법령 기준)
    // 예: 2026-08-17 광복절 대체, 2026-09-28 추석 대체, 2026-10-05 개천절 대체
    for (const [d, n] of Object.entries(FALLBACK)) {
      if (d.startsWith(`${year}-`) && n.endsWith('대체') && !out[d]) {
        out[d] = n;
      }
    }
    return out;
